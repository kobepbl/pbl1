...
class Article extends DbData{
    ...
    public function Insertarticle($user_id,$title,$sentence,$like_count,$creation_date){
        $sql = "select max(article_id) from article_list";
        $stmt = $this->query($sql);
        $article_id = $stmt->fetch();// 抽出したデータを取り出す 
        $article_id =$article_id+1;
        $sql = "insert into users(article_id,user_id,title,sentence,like_count,creation_date) values(?, ?, ?, ?, ?, ?)";
        $result = $this->exec($sql,[$article_id,$user_id,$title,$sentence,$like_count,$creation_date]);
        
        if($result){
            return ''; // ここも空文字を返すので「''」はシングルクォーテーションが２つ
        } else {
            // 何らかの原因で失敗した場合 
            return '新規登録できませんでした。管理者にお問い合わせください。';
        }
    }
    /*
